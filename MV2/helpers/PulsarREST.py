...
def get_tenants(pulsar_admin_url=None):
    ...
    try:
        ...
        print(f"Is Pulsar running? {datetime.datetime.now()}: {err}")
        sys.stdout.flush()
        time.sleep(10)
        get_tenants(pulsar_admin_url)
...
def get_namespaces(pulsar_admin_url=None, tenant=None):
    ...
    try:
        r = requests.get(f"{pulsar_admin_url}/namespaces/{tenant}",
                         headers={'Content-Type': "application/json",
                                  'Accept': "application/json"})
        # print(r.request.url)
        if (r.status_code == 200):
        ...
        print(f"PulsarREST.py - get_namespaces: ERROR: {e}")
        return {}
