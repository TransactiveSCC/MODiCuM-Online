...
class Client:
    ...
    def createAllocation(self, allocation_uuid, alloc_msg):
        ...
        cprint(f"{self.cfg.user_uuid} createAllocation alloc_msg.price: {alloc_msg.price} "
               f"{self.cfg.user_uuid} createAllocation type(alloc_msg.price): {type(alloc_msg.price)}", "yellow")

        cprint(f"{self.cfg.user_uuid} createAllocation allocation: {alloc_msg}", "yellow")
        cprint(f"{self.cfg.user_uuid} createAllocation type(allocation) {type(alloc_msg.allocation)}", "yellow")
        sys.stdout.flush()
        ...
        ledger_msg = MV2.schema.LedgerSchema(
            ...
            args=alloc_msg.allocation,
            args_bytes=[b'0'],
            timestamp=datetime.datetime.now(tz=datetime.timezone.utc).timestamp(),
            # allocation=alloc_msg.allocation
        )

        cprint(f"{self.cfg.user_uuid} supplier_uuids in influx log {alloc_msg.supplier_uuids}", "yellow")
        cprint(f"{self.cfg.user_uuid} send createAllocation to ledger", "yellow")
        # log_msg = {"fields": {"ledger": self.cfg.State.createAllocation.value},
        #            "tags": {"offer_uuid": "customer_1_offer_0"}
        #            }
        ...
        cprint(f"{self.cfg.user_uuid} done sending createAllocation to ledger", "yellow")
        sys.stdout.flush()
    ...
    def postOutput(self, allocation_uuid, user_uuid, hashed_output_checklist, input_MessageId_list):
        cprint(f"{self.cfg.user_uuid} Supplier commits output hash to the ledger", "yellow")
        cprint(f"{self.cfg.user_uuid} {input_MessageId_list}", "yellow")
